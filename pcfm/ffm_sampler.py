...
class FFM_NS_sampler:
    ...
    # our method, PCFM
    def pcfm_sample(self, u0, n_step, hfunc, mode='root', newtonsteps=1, eps=1e-6,
                    guided_interpolation=True, interpolation_params={}, pc_only_last_step=False):
        dt = 1.0 / n_step
        u = u0.clone()
        ts = torch.linspace(0, 1, n_step + 1, device=u0.device)[:-1]
        for idx, t in enumerate(tqdm(ts, desc="PCFM sampling")):
            vf = self.model(t, u)
            if pc_only_last_step and idx != (n_step - 1):
                u = u + dt * vf
                continue
