...
def ns_equation_residual(u_flat, nx, ny, nt, device, visc=1e-3):
    ...
    for n in range(1, nt - 1):
        ...
        # Already have Laplacian in Fourier space
        laplacian_h = -lap * w_h
        laplacian = torch.fft.ifftn(laplacian_h, dim=[0, 1], norm='backward').real
        diffusion = visc * laplacian
        
        # 6) Forcing term (from dataset generation)
        # f(x,y) = 0.1 * √2 * sin(2π(x+y) + φ)
        # For residual calculation, assume f=0 or extract from data
        # Since forcing is constant in time, we approximate as 0 for simplicity
        # (or you can pass it as a parameter if known)
        forcing = 0.0
    ...
    # Concatenate all time point residuals
...
u0 = model.gp.sample(grid, (nx,ny,nt), n_samples=1).to(device)
del grid
torch.cuda.empty_cache()
...
def lambda_schedule(t):
    ...
    """
    # Linear increase from 0 to 1e0
    # return t * 1e0
    # Exponential: lam_0 * exp(k*t), scaled so at t=1 we get ~1e0
    # exp(5*t) gives exp(5)≈148 at t=1, so scale it down
    # 1e-3 * exp(5*t) gives ~0.15 at t=1 (still small)
    # 1e-2 * exp(5*t) gives ~1.5 at t=1 (good balance)
...
# Fixed lambda params (standard PCFM)
interpolation_params_fixed = {
    'custom_lam': 1e0,  # Fixed lambda throughout
    'step_size': 1e-2,
    'num_steps': 20
}

# Increasing lambda params (our experiment)
interpolation_params_increasing = {
    # 'custom_lam': 1e0,  # Will be overridden by schedule
    'custom_lam': 1e-2,  # Initial lambda value
    'step_size': 1e-2,
    'num_steps': 20,
...
with torch.no_grad():
    u_vanilla = sampler.vanilla_sample(u0, n_step=args.n_step)
...
pcfm_increasing_metrics = evaluate_ns_constraints(u_pcfm_increasing[0], residuals)
...
torch.save(u_vanilla.cpu(), os.path.join(args.outdir, "u_vanilla.pt"))
torch.save(u_pcfm_fixed.cpu(), os.path.join(args.outdir, "u_pcfm_fixed.pt"))
torch.save(u_pcfm_increasing.cpu(), os.path.join(args.outdir, "u_pcfm_increasing.pt"))

plot_slice(u_true, "Ground Truth", os.path.join(args.outdir,"gt.png"))
plot_slice(u_vanilla[0], "Vanilla FFM", os.path.join(args.outdir,"vanilla.png"))
plot_slice(u_pcfm_fixed[0], "PCFM (λ=1e0)", os.path.join(args.outdir,"pcfm_fixed.png"))
plot_slice(u_pcfm_increasing[0], "PCFM (λ: 0→1e0)", os.path.join(args.outdir,"pcfm_increasing.png"))
